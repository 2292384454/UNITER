...
class UniterConfig(object):
    ...
    def __init__(self,
                 ...
                 num_hidden_layers=12,  # Transformer 编码器中的隐藏层数。
                 num_attention_heads=12,  # Transformer 编码器中每个attention layer 的 attention heads 数量。
                 intermediate_size=3072,  # Transformer 编码器中“中间”（即前馈）层的 size。
                 hidden_act="gelu",  # 编码器和池化器中的非线性激活函数("gelu","relu","swish")。
                 hidden_dropout_prob=0.1,  # 嵌入、编码器和池化器中所有全连接层的 dropout 概率。
                 attention_probs_dropout_prob=0.1,  # attention 中的 drop 概率。
                 max_position_embeddings=512,  # 最大序列长度，通常将其设置为较大的值以防万一（如 512 或 1024 或 2048）。
                 type_vocab_size=2,  # token_type_ids 的词表 size 。
...
class UniterTextEmbeddings(nn.Module):
    """
    KevinHwang: 此类的作用是将训练语料中的 text 信息转化成 embedding 向量，
    embedding 向量的构成分为三个部分 words_embeddings（词嵌入向量）、
    position_embeddings（位置嵌入向量）、token_type_embeddings
    （是否为前后语句对嵌入向量）。
    """
...
class UniterImageEmbeddings(nn.Module):
    """
    KevinHwang: Uniter 模型中 Image 转化为 embedding 向量是关键，
    img_feat 和 img_pos_feat 是如何生成的可以去论文 3.1 小节看一下，
    简单来说就是使用 Faster R-CNN 检测出多个 region，region feature
    特征经过 reshape 或者 dnn 等方式转为 img_feat ，region 的 location
    转为 img_pos_feat 。然后分别经过 fc 层至相同维度，相加再经过 LN 层就生
    成了既含有图片特征又含有位置信息的 embedding。
    """
